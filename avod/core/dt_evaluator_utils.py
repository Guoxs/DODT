...
def recovery_coordinate(dataset, samples_names, predictions):
    ...
    if len(predictions) != 0:
        for j in range(len(predictions)):
            ...
            obj = ObjectLabel()
            obj.t = predictions[j][0:3]
            obj.l = predictions[j][3]
            obj.w = predictions[j][4]
            obj.h = predictions[j][5]
            obj.ry = predictions[j][6]

            obj.t = dataset.recovery_t(obj, calib, trans, matrix)
            obj.ry -= delta

            # convert back to numpy
            predictions[j][0:3] = obj.t
            predictions[j][6] = obj.ry
    return predictions
...
def interpolate_non_keyframe_predicitons(dataset, sample_names, predictions, threshold):
    def cal_iou(box3d_1, box3d_2):
        ...
        box3d = box3d_1[[6, 3, 5, 4, 0, 1, 2]]
        if len(box3d_2.shape) == 1:
            boxes3d = box3d_2[[6, 3, 5, 4, 0, 1, 2]]
        else:
            boxes3d = box3d_2[:, [6, 3, 5, 4, 0, 1, 2]]
    ...
    if num  == 1:
        # [x, y, z, l, w, h, r, score, type]
        preds = pred_lists[0]
        final_predictions = [preds[np.where(preds[:, 7] > threshold)[0]][:, :-4]]
    ...
    if num == 2:
        ...
        preds = [pred[np.where(pred[:, 7] > threshold)[0]] for pred in pred_lists]
        final_predictions = [pred[:, :-4] for pred in preds]
    ...
    # more than 3 frames
        ...
        kept_list = [pred[np.where(pred[:, 7] > threshold)[0]] for pred in pred_lists]
        # object match
        trajectories = []
...
def interpolate_trajectory(trajectories, num):
    ...
    for track in trajectories:
        ...
        if track[0] != [] and track[1] != []:
            ...
            offsets = track[1][[0, 2, 6]] - track[0][[0, 2, 6]]
            score = max(track[0][7], track[1][7])
            for i in range(num-2):
                new_obj = copy.deepcopy(track[0])
                new_obj[[0, 2, 6]] += offsets * (i + 1.0) / (num - 1)
                new_obj[7] = score
            ...
            if d <= track[1][4] / 2:
                ...
                for i in range(num-1):
                    ...
                    new_obj[0] -= delta_x * (num - i - 2) / (num - 1)
                    new_obj[2] -= delta_z * (num - i - 2)  / (num - 1)
                    new_track.append(new_obj)
                new_track.append(track[1])
            else:
            ...
            if d <= track[0][4] / 2:
                ...
                for i in range(num - 1):
                    new_obj = copy.deepcopy(track[0])
                    new_obj[0] += delta_x * (i + 1.0) / (num - 1)
                    new_obj[2] += delta_z * (i + 1.0) / (num - 1)
                    new_track.append(new_obj)
            else:
                new_track.append(track[0])
