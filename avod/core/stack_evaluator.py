...
class StackEvaluator:
    ...
    def __init__(self,
        ...
        self.full_model = isinstance(self.model, StackAvodModel)
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            if validation:
                ...
                rpn_objectness_loss = eval_losses[StackRpnModel.LOSS_RPN_OBJECTNESS]
                rpn_regression_loss = eval_losses[StackRpnModel.LOSS_RPN_REGRESSION]
                ...
                # Save proposals
                proposals_and_scores = self.get_rpn_proposals_and_scores(predictions)
    ...
    def _update_avod_box_cls_loc_orient_losses(self,
        ...
        avod_orientation_loss = \
            eval_losses[StackAvodModel.LOSS_FINAL_ORIENTATION]
    ...
    def get_cls_accuracy(self,
        ...
        objectness_pred = predictions[StackRpnModel.PRED_MB_OBJECTNESS]
        objectness_gt = predictions[StackRpnModel.PRED_MB_OBJECTNESS_GT]
        ...
        if self.full_model:
            classification_pred = \
                predictions[StackAvodModel.PRED_MB_CLASSIFICATION_SOFTMAX]
            classification_gt = \
                predictions[StackAvodModel.PRED_MB_CLASSIFICATIONS_GT]
    ...
    def get_avod_predicted_boxes_3d_and_scores(self, predictions, box_rep):
        ...
        if box_rep == 'box_3d':
            ...
            final_pred_anchors = predictions[StackAvodModel.PRED_TOP_PREDICTION_ANCHORS]
            final_pred_orientations = predictions[StackAvodModel.PRED_TOP_ORIENTATIONS]
            ...
            final_pred_boxes_3d = predictions[StackAvodModel.PRED_TOP_PREDICTION_BOXES_3D]
        ...
        # anchor index
        anchor_top_k_indices = predictions[StackAvodModel.PRED_RPN_ANCHORS_TOP_K_IDX]
        ...
        for i in range(size):
            ...
            predictions_and_scores[i] = np.column_stack(
                [anchor_top_k_indices[i][:, np.newaxis],
    ...
    def get_avod_predicted_box_corners_and_scores(self, predictions, box_rep):
        ...
        if box_rep in ['box_8c', 'box_8co']:
            final_pred_box_corners = predictions[StackAvodModel.PRED_TOP_BOXES_8C]

        elif box_rep in ['box_4c', 'box_4ca']:
            final_pred_box_corners = predictions[StackAvodModel.PRED_TOP_BOXES_4C]
    ...
    def run_kitti_native_eval(self, global_step):
        ...
        # Store predictions in kitti format
        evaluator_utils.save_stack_predictions_in_kitti_format(
            self.model,
            self.model_config.checkpoint_name,
