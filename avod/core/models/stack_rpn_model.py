...
class StackRpnModel(model.DetectionModel):
    ##############################
    ...
    ##############################
    # Keys for Integrated Data
    ##############################
    PL_INTEGRATED_BEV_INPUT = 'integrated_bev_input_pl'

    PL_INTEGRATED_ANCHORS = 'integrated_anchors_pl'
    PL_INTEGRATED_ANCHOR_IOUS = 'integrated_anchors_ious_pl'
    PL_INTEGRATED_ANCHOR_OFFSETS = 'integrated_anchors_offsets_pl'
    PL_INTEGRATED_ANCHOR_CLASSES = 'integrated_anchors_classes_pl'

    PL_INTEGRATED_BEV_ANCHORS = 'integrated_bev_anchors_pl'
    PL_INTEGRATED_BEV_ANCHORS_NORM = 'integrated_bev_anchors_norm_pl'
    PL_INTEGRATED_IMG_ANCHORS = 'integrated_img_anchors_pl'
    PL_INTEGRATED_IMG_ANCHORS_NORM = 'integrated_img_anchors_norm_pl'

    PL_INTEGRATED_LABEL_ANCHORS = 'integrated_label_anchors_pl'
    ...
    PL_CORRELATION_COEXISTS = 'correlation_coexists_pl'
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        super(StackRpnModel, self).__init__(model_config)
        ...
        self.sample_num = self.dataset.sample_num
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('bev_input'):
            ...
            bev_input_placeholder_couple = self._add_placeholder(tf.float32,
                                                                 all_bev_dims, self.PL_BEV_INPUT)
        ...
        with tf.variable_scope('img_input'):
            ...
            img_input_placeholder_couple = self._add_placeholder(
                                                tf.float32,
                                                [self.sample_num, None, None, self._img_depth],
                                                self.PL_IMG_INPUT)
            ...
            # Summary Image
            tf.summary.image("rgb_image", self._img_preprocessed[0], max_outputs=2)

        with tf.variable_scope('integrated_bev_input'):
            integrated_bev_input_placeholder = \
                self._add_placeholder(tf.float32, bev_dims, self.PL_INTEGRATED_BEV_INPUT)
            self._integrated_bev_input_batches = tf.expand_dims(
                integrated_bev_input_placeholder, axis=0)
            self._integrated_bev_preprocessed = self._bev_feature_extractor.preprocess_input(
                                    self._integrated_bev_input_batches, self._bev_pixel_size)
            # Summary Images
            integrated_bev_summary_images = tf.split(
                integrated_bev_input_placeholder, self._bev_depth, axis=2)
            tf.summary.image("integrated_bev_maps", integrated_bev_summary_images,
        ...
        with tf.variable_scope('pl_labels'):
            ...
            for i in range(self.sample_num):
                ...
                self._add_placeholder(tf.int32, [None], name)

        # Placeholders for integrated anchors
        with tf.variable_scope('pl_integrated_labels'):
            self._add_placeholder(tf.float32, [None, 6], self.PL_INTEGRATED_LABEL_ANCHORS)
            self._add_placeholder(tf.float32, [None, 7], self.PL_INTEGRATED_LABEL_BOXES_3D)
            self._add_placeholder(tf.float32, [None], self.PL_INTEGRATED_LABEL_CLASSES)

        # Placeholders for integrated_anchors
        with tf.variable_scope('pl_integrated_anchors'):
            self._add_placeholder(tf.float32, [None, 6], self.PL_INTEGRATED_ANCHORS)
            self._add_placeholder(tf.float32, [None], self.PL_INTEGRATED_ANCHOR_IOUS)
            self._add_placeholder(tf.float32, [None, 6], self.PL_INTEGRATED_ANCHOR_OFFSETS)
            self._add_placeholder(tf.float32, [None], self.PL_INTEGRATED_ANCHOR_CLASSES)

            with tf.variable_scope('integrated_bev_anchor_projections'):
                self._add_placeholder(tf.float32, [None, 4],
                                      self.PL_INTEGRATED_BEV_ANCHORS)
                self._integrated_bev_anchors_norm_pl = self._add_placeholder(
                            tf.float32, [None, 4], self.PL_INTEGRATED_BEV_ANCHORS_NORM)
            with tf.variable_scope('integrated_img_anchor_projections'):
                self._add_placeholder(tf.float32, [None, 4],
        ...
        with tf.variable_scope('pl_correlation_offsets'):
            ...
            # [0,0,1, -1, 0,...], 0 means co-exist, 1 means only exist in frame1, -1 means only exist in frame0
            self._add_placeholder(tf.float32, [None], self.PL_CORRELATION_COEXISTS)
    ...
    def _set_up_feature_extractors(self):
        ...
        """
        self.bev_feature_maps = [None] * self.sample_num
        self.bev_end_points = [None] * self.sample_num
        self.img_feature_maps = [None] * self.sample_num
        self.img_end_points = [None] * self.sample_num
        ...
        with tf.variable_scope('feature_extractor') as scope:
            ...
            # integrated bev feature extractor
            self.integrated_bev_feature_map, \
            self.integrated_bev_end_points = self._bev_feature_extractor.build(
                        self._integrated_bev_preprocessed,
                        self._bev_pixel_size,
                        self._is_training)
        ...
        with tf.variable_scope('bev_bottleneck') as scope:
            ...
            for bev_feature_map in self.bev_feature_maps:
                ...
                scope.reuse_variables()
            # for integrated_bev_feature_map
            self.integrated_bev_bottleneck = slim.conv2d(
                        self.integrated_bev_feature_map,
                        1, [1, 1],
                        scope='bottleneck',
                        normalizer_fn=slim.batch_norm,
    ...
    def build(self):
        ...
        bev_proposal_input = self.integrated_bev_bottleneck
        img_proposal_input = self.img_bottleneck[0]
        ...
        with tf.variable_scope('proposal_roi_pooling'):
            with tf.variable_scope('box_indices'):
                ...
                bev_boxes_norm_batches = tf.expand_dims(
                    self._integrated_bev_anchors_norm_pl, axis=0)
            ...
            bev_proposal_rois = tf.image.crop_and_resize(
                bev_proposal_input,
                self._integrated_bev_anchors_norm_pl,
                tf_box_indices,
                self._proposal_roi_crop_size)
            # Do ROI Pooling on image
            img_proposal_rois = tf.image.crop_and_resize(
                img_proposal_input,
                self._integrated_img_anchors_norm_pl,
                tf_box_indices,
                self._proposal_roi_crop_size)
        ...
        with tf.variable_scope('histograms_feature_extractor'):
            with tf.variable_scope('bev_vgg'):
                for end_point in self.integrated_bev_end_points:
                    tf.summary.histogram(
                        end_point, self.integrated_bev_end_points[end_point])

            with tf.variable_scope('img_vgg'):
                for end_point in self.img_end_points[0]:
                    tf.summary.histogram(
                        end_point, self.img_end_points[0][end_point])
        ...
        with tf.variable_scope('proposals'):
            anchors = self.placeholders[self.PL_INTEGRATED_ANCHORS]
            ...
            with tf.variable_scope('nms'):
                ...
                top_objectness_softmax = tf.gather(objectness_scores,
                                                   top_indices)
        # Get mini batch
        all_ious_gt = self.placeholders[self.PL_INTEGRATED_ANCHOR_IOUS]
        all_offsets_gt = self.placeholders[self.PL_INTEGRATED_ANCHOR_OFFSETS]
        all_classes_gt = self.placeholders[self.PL_INTEGRATED_ANCHOR_CLASSES]

        with tf.variable_scope('mini_batch'):
            mini_batch_utils = self.dataset.kitti_utils.mini_batch_utils
            mini_batch_mask, _ = mini_batch_utils.sample_rpn_mini_batch(all_ious_gt)
        ...
        with tf.variable_scope('bev_rpn_rois'):
            mb_bev_anchors_norm = tf.boolean_mask(
                self._integrated_bev_anchors_norm_pl, mini_batch_mask)
            mb_bev_box_indices = tf.zeros_like(
                tf.boolean_mask(all_classes_gt, mini_batch_mask), dtype=tf.int32)
        ...
        with tf.variable_scope('img_rpn_rois'):
            # ROIs on image input
            mb_img_anchors_norm = tf.boolean_mask(self._integrated_img_anchors_norm_pl,
                                                  mini_batch_mask)
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        if self._train_val_test in ["train", "val"]:
            ...
            while not valid_sample:
                ...
                anchors_info = couple_sample.get(constants.KEY_INTEGRATED_ANCHORS_INFO)
        ...
        # Integrated data
        integrated_bev_input = couple_sample.get(constants.KEY_INTEGRATED_BEV_INPUT)
        integrated_label_anchors = couple_sample.get(constants.KEY_INTEGRATED_LABEL_ANCHOR)
        ...
        corr_coexists = couple_sample.get(constants.KEY_CORR_COEXISTS)
        ...
        self._placeholder_inputs[self.PL_INTEGRATED_BEV_INPUT] = integrated_bev_input
        ...
        self._placeholder_inputs[self.PL_CORRELATION_OFFSETS] = corr_offsets
        self._placeholder_inputs[self.PL_CORRELATION_COEXISTS] = corr_coexists
    ...
    def _fill_anchor_pl_inputs(self,
        ...
        self._placeholder_inputs[self.PL_INTEGRATED_ANCHORS] = anchors_to_use
        ...
        if self._train_val_test in ['train', 'val'] and len(anchors_ious) > 0:
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_IOUS] = anchors_ious
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_OFFSETS] = anchor_offsets
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_CLASSES] = anchor_classes
        ...
        # During test, or val when there is no anchor info
            ...
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_IOUS] = np.zeros(num_anchors)
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_OFFSETS] = np.zeros([num_anchors, 6])
            self._placeholder_inputs[self.PL_INTEGRATED_ANCHOR_CLASSES] = np.zeros(num_anchors)
        else:
            raise ValueError('Got run mode {}, and non-empty anchor info'.
                             format(self._train_val_test))

        self._placeholder_inputs[self.PL_INTEGRATED_BEV_ANCHORS] = bev_anchors
        self._placeholder_inputs[self.PL_INTEGRATED_BEV_ANCHORS_NORM] = self._bev_anchors_norm
        self._placeholder_inputs[self.PL_INTEGRATED_IMG_ANCHORS] = img_anchors
        self._placeholder_inputs[self.PL_INTEGRATED_IMG_ANCHORS_NORM] = self._img_anchors_norm
