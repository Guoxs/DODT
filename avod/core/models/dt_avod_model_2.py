...
from avod.builders import avod_fc_layers_builder, avod_corr_layers_builder
from avod.builders import dt_avod_loss_builder
...
class DtAvodModel(model.DetectionModel):
    ##############################
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        super(DtAvodModel, self).__init__(model_config)
        ...
        # Create the RpnModel
        self._rpn_model = DtRpnModel(model_config, train_val_test, dataset)
    ...
    def build(self):
        ...
        with tf.variable_scope('avod_projection'):
            ...
            if self._config.expand_proposals_xz > 0.0:
                ...
                with tf.variable_scope('expand_xz'):
                    expanded_dim_x = [top_anchors[i][:, 3] + expand_length
                                      for i in range(SAMPLE_SIZE)]
                    expanded_dim_z = [top_anchors[i][:, 5] + expand_length
                                      for i in range(SAMPLE_SIZE)]
                    ...
                    expanded_anchors = [tf.stack([
                        ...
                        top_anchors[i][:, 4],
                        expanded_dim_z[i]
                    ], axis=1) for i in range(SAMPLE_SIZE)]
            ...
            with tf.variable_scope('bev'):
                bev_proposal_boxes = [None]*SAMPLE_SIZE
                bev_proposal_boxes_norm = [None]*SAMPLE_SIZE
                bev_proposal_boxes_tf_order = [None]*SAMPLE_SIZE
                bev_proposal_boxes_norm_tf_order = [None]*SAMPLE_SIZE
                for i in range(SAMPLE_SIZE):
                    # Project top anchors into bev and image spaces
                    bev_proposal_boxes[i], bev_proposal_boxes_norm[i] = \
                        anchor_projector.project_to_bev(
                            avod_projection_in[i],
                            self.dataset.kitti_utils.bev_extents)

                    # Reorder projected boxes into [y1, x1, y2, x2]
                    bev_proposal_boxes_tf_order[i] = \
                        anchor_projector.reorder_projected_boxes(
                            bev_proposal_boxes[i])
                    bev_proposal_boxes_norm_tf_order[i] = \
                        anchor_projector.reorder_projected_boxes(
                            bev_proposal_boxes_norm[i])

            with tf.variable_scope('img'):
                image_shape = [None]*SAMPLE_SIZE
                img_proposal_boxes = [None]*SAMPLE_SIZE
                img_proposal_boxes_norm = [None]*SAMPLE_SIZE
                img_proposal_boxes_norm_tf_order = [None]*SAMPLE_SIZE
                for i in range(SAMPLE_SIZE):
                    image_shape[i] = tf.cast(tf.shape(
                        rpn_model.placeholders[DtRpnModel.PL_IMG_INPUT][i])[0:2],
                        tf.float32)
                    img_proposal_boxes[i], img_proposal_boxes_norm[i] = \
                        anchor_projector.tf_project_to_image_space(
                            avod_projection_in[i],
                            rpn_model.placeholders[DtRpnModel.PL_CALIB_P2],
                            image_shape[i])
                    # Only reorder the normalized img
                    img_proposal_boxes_norm_tf_order[i] = \
                        anchor_projector.reorder_projected_boxes(
                            img_proposal_boxes_norm[i])
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            bev_boxes_norm_batches = [tf.expand_dims(bev_proposal_boxes_norm[i], axis=0)
                                      for i in range(SAMPLE_SIZE)]
            ...
            bev_rois = [tf.image.crop_and_resize(
                ...
                tf_box_indices[i],
                self._proposal_roi_crop_size,
                name='bev_rois') for i in range(SAMPLE_SIZE)]
        ...
        with tf.variable_scope('avod_fc_layer') as scope:
            fc_output_layers = [None] * SAMPLE_SIZE
            for i in range(SAMPLE_SIZE):
                fc_output_layers[i] = \
        ...
        # This may be None
        all_angle_vectors = \
            [fc_output_layers[i].get(avod_fc_layers_builder.KEY_ANGLE_VECTORS)
             for i in range(SAMPLE_SIZE)]
        ...
        corr_offsets_gt = rpn_model.placeholders[DtRpnModel.PL_CORR_ANCHORS_OFFSETS]
        ...
        with tf.variable_scope('avod_gt_projection'):
            bev_anchor_boxes_gt = [None]*SAMPLE_SIZE
            bev_anchor_boxes_gt_tf_order = [None]*SAMPLE_SIZE
            for i in range(SAMPLE_SIZE):
                bev_anchor_boxes_gt[i], _ = anchor_projector.project_to_bev(
                    anchors_gt[i], self.dataset.kitti_utils.bev_extents)

                bev_anchor_boxes_gt_tf_order[i] = \
                    anchor_projector.reorder_projected_boxes(bev_anchor_boxes_gt[i])

        with tf.variable_scope('avod_box_list'):
            # Convert to box_list format
            anchor_box_list_gt = [box_list.BoxList(bev_anchor_boxes_gt_tf_order[i])
                                  for i in range(SAMPLE_SIZE)]
            anchor_box_list = [box_list.BoxList(bev_proposal_boxes_tf_order[i])
                               for i in range(SAMPLE_SIZE)]


        mb_mask = [None] * SAMPLE_SIZE
        mb_class_label_indices = [None] * SAMPLE_SIZE
        mb_gt_indices = [None] * SAMPLE_SIZE
        for i in range(SAMPLE_SIZE):
            mb_mask[i], mb_class_label_indices[i], mb_gt_indices[i] = \
                self.sample_mini_batch(
                    anchor_box_list_gt=anchor_box_list_gt[i],
                    anchor_box_list=anchor_box_list[i],
        ...
        with tf.variable_scope('avod_apply_mb_mask'):
            ...
            mb_classifications_logits = [tf.boolean_mask(
                all_cls_logits[i], mb_mask[i]) for i in range(SAMPLE_SIZE)]
            mb_classifications_softmax = [tf.boolean_mask(
                all_cls_softmax[i], mb_mask[i]) for i in range(SAMPLE_SIZE)]

            # Offsets
            mb_offsets = [tf.boolean_mask(all_offsets[i], mb_mask[i]) for i in range(SAMPLE_SIZE)]

            # Angle Vectors
            mb_angle_vectors = [None]*SAMPLE_SIZE
            for i in range(SAMPLE_SIZE):
                if all_angle_vectors[i] is not None:
                    mb_angle_vectors[i] = tf.boolean_mask(all_angle_vectors[i], mb_mask[i])
                else:
        ...
        with tf.variable_scope('avod_encode_mb_anchors'):
            mb_anchors_gt = [None]*SAMPLE_SIZE
            mb_offsets_gt = [None]*SAMPLE_SIZE
            mb_orientations_gt = [None]*SAMPLE_SIZE
            proposal_boxes_8c = [None]*SAMPLE_SIZE
            proposal_boxes_4c = [None]*SAMPLE_SIZE

            # Gather corresponding ground truth corr offsets for each mb sample
            mb_corr_offsets_gt = tf.gather(corr_offsets_gt, mb_gt_indices[0])
            ...
            for i in range(SAMPLE_SIZE):
                ...
                if self._box_rep == 'box_3d':
                    ...
                    # mb sample
                    mb_orientations_gt[i] = tf.gather(orientations_gt[i],
                                                   mb_gt_indices[i])

                elif self._box_rep in ['box_8c', 'box_8co']:

                    # Get boxes_3d ground truth mini-batch and convert to box_8c
                    mb_boxes_3d_gt = tf.gather(boxes_3d_gt[i], mb_gt_indices[i])
                    ...
                    mb_boxes_8c = tf.boolean_mask(proposal_boxes_8c[i], mb_mask[i])
                    mb_offsets_gt[i] = box_8c_encoder.tf_box_8c_to_offsets(
                    ...
                    if i == 0:
                        mb_corr_boxes_3d_gt = box_3d_encoder.anchors_to_box_3d(
                            mb_corr_offsets_gt, fix_lw=True)
                        mb_corr_boxes_8c_gt = box_8c_encoder.tf_box_3d_to_box_8c(
                            mb_corr_boxes_3d_gt)
                        mb_corr_offsets_gt = tf.reshape(mb_corr_boxes_8c_gt, [-1, 24])

                elif self._box_rep in ['box_4c', 'box_4ca']:

                    # Get ground plane for box_4c conversion
                    ground_plane = self._rpn_model.placeholders[
                        self._rpn_model.PL_GROUND_PLANE][i]
        ...
        with tf.variable_scope('bev_avod_rois'):
            for i in range(SAMPLE_SIZE):
                mb_bev_anchors_norm = tf.boolean_mask(
                    bev_proposal_boxes_norm_tf_order[i], mb_mask[i])
                mb_bev_box_indices = tf.zeros_like(mb_gt_indices[i], dtype=tf.int32)

                # Show the ROIs of the BEV input density map
                # for the mini batch anchors
                bev_input_rois = tf.image.crop_and_resize(
                    self._rpn_model._bev_preprocessed[i],
                    mb_bev_anchors_norm,
                    mb_bev_box_indices,
                    (32, 32))
        ...
        with tf.variable_scope('img_avod_rois'):
            for i in range(SAMPLE_SIZE):
                ...
                mb_img_anchors_norm = tf.boolean_mask(
                    img_proposal_boxes_norm_tf_order[i], mb_mask[i])
                mb_img_box_indices = tf.zeros_like(mb_gt_indices[i], dtype=tf.int32)

                # Do test ROI pooling on mini batch
                img_input_rois = tf.image.crop_and_resize(
                    self._rpn_model._img_preprocessed[i],
                    mb_img_anchors_norm,
                    mb_img_box_indices,
                    (32, 32))
        ...
        top_classification_logits = [None]*SAMPLE_SIZE
        top_classification_softmax = [None]*SAMPLE_SIZE
        top_prediction_anchors = [None]*SAMPLE_SIZE
        top_orientations = [None]*SAMPLE_SIZE
        top_prediction_boxes_3d = [None]*SAMPLE_SIZE
        ...
        for i in range(SAMPLE_SIZE):
            ...
            with tf.variable_scope('avod_regression'):
                if self._box_rep == 'box_3d':
                    ...
                    reshaped_offsets = tf.reshape(all_offsets[i],
                                                  [-1, 3, 8])
                    # Given the offsets, get the boxes_8c
                    prediction_boxes_8c = \
                        box_8c_encoder.tf_offsets_to_box_8c(proposal_boxes_8c[i],
                                                            reshaped_offsets)
                    ...
                    prediction_boxes_4c = \
                        box_4c_encoder.tf_offsets_to_box_4c(proposal_boxes_4c[i],
                                                            all_offsets[i])
            ...
            with tf.variable_scope('avod_nms'):
                ...
                all_top_scores = tf.reduce_max(all_cls_logits[i][:, 1:], axis=1)
    ...
    def loss(self, prediction_dict):
        ...
        losses_output = dt_avod_loss_builder.build(self, prediction_dict)

        classification_loss = tf.reduce_sum(
            losses_output[dt_avod_loss_builder.KEY_CLASSIFICATION_LOSS])
        ...
        offset_loss_norm = tf.reduce_sum(
            losses_output[dt_avod_loss_builder.KEY_OFFSET_LOSS_NORM])
        ...
        ang_loss_loss_norm = losses_output.get(dt_avod_loss_builder.KEY_ANG_LOSS_NORM)
        if ang_loss_loss_norm is not None:
            ang_loss_loss_norm = tf.reduce_sum(ang_loss_loss_norm)
