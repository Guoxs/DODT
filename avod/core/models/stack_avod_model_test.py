...
class StackAvodModelTest(tf.test.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.dataset = DatasetBuilder.build_kitti_tracking_stack_dataset(dataset_config)

    def test_avod_loss(self):
        # tests the set up for the model and the loss
        # Use "val" so that the first sample is loaded each time
        avod_model = StackAvodModel(self.model_config,
                               train_val_test="val",
