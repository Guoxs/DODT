...
def _get_correlation_loss(model, corr_offsets, corr_offsets_gt,
                          cls_softmax, cls_gt):
    ...
    anchorwise_correlation_loss = weighted_smooth_l1_loss(
        corr_offsets, corr_offsets_gt, weight=corr_loss_weight)
    ...
    # Apply mask to only keep correlation loss for positive predictions
    pos_correlation_loss = tf.reduce_sum(tf.boolean_mask(
        anchorwise_correlation_loss, pos_classification_mask))
    ...
    with tf.variable_scope('reg_norm'):
        ...
        corr_loss = tf.cond(
            pos_div_cond,
            lambda: pos_correlation_loss / num_positives,
...
def _build_cls_off_ang_loss(model, prediction_dict):
    ...
    with tf.variable_scope('avod_losses'):
        ...
        with tf.variable_scope('regression'):
            final_reg_loss = [None] * size
            offset_loss_norm = [None] * size
            ang_loss_norm = [None] * size
            for i in range(size):
