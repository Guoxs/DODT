...
from avod.core.stack_evaluator import StackEvaluator
...
def evaluate(model_config, eval_config, dataset_config):
    ...
    with tf.Graph().as_default():
        if model_name == 'stack_avod_model':
            model = StackAvodModel(model_config, train_val_test=eval_mode,
                              dataset=dataset)
        elif model_name == 'stack_rpn_model':
            model = StackRpnModel(model_config, train_val_test=eval_mode,
                             dataset=dataset)
        else:
            raise ValueError('Invalid model name {}'.format(model_name))

        model_evaluator = StackEvaluator(model,
                                    dataset_config,
                                    eval_config)
