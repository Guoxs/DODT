...
def do_preprocessing(dataset, indices):
    ...
    mini_batch_utils.preprocess_rpn_mini_tracking_batches(indices)
...
def main(dataset=None):
    ...
    ##############################
    if process_car:
        car_dataset = DatasetBuilder.load_tracking_dataset_from_config(
            car_dataset_config_path)
    if process_ped:
        ped_dataset = DatasetBuilder.load_tracking_dataset_from_config(
            ped_dataset_config_path)
    if process_cyc:
        cyc_dataset = DatasetBuilder.load_tracking_dataset_from_config(
            cyc_dataset_config_path)
    if process_ppl:
        ppl_dataset = DatasetBuilder.load_tracking_dataset_from_config(
